...
class ValidatorBurnDataSource(IBurnDataSource):
    ...
    def _fetch_total_sales_usd(self, scope: str) -> Optional[float]:
        ...
        Fetches from the /miner-stats endpoint which returns total_sales_usd per scope.
        ...
        """
        try:
            # Get window_days for this scope to pass to the API
            window_days = self.window_days_getter(scope)
            ...
            url = f"{api_base_url}/miner-stats"
            ...
            # total_sales_usd is a dict mapping scope to value
            total_sales_by_scope = data.get("total_sales_usd", {})
            total_sales = total_sales_by_scope.get(scope)
            
            if total_sales is None:
                logging.warning(f"total_sales_usd not found for scope {scope} in API response")
                return None
            
            logging.info(f"Fetched total_sales_usd: ${total_sales:.2f} for scope {scope}")
            return float(total_sales)
            
        except requests.exceptions.RequestException as e:
            logging.warning(f"Failed to fetch total_sales_usd from API for scope {scope}: {e}")
            return None
        except (ValueError, KeyError, TypeError) as e:
            logging.warning(f"Failed to parse total_sales_usd from API response for scope {scope}: {e}")
            return None
